...
// MasterChef is the master of Age. He can make Age and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    function pendingAge(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accAgePerShare = accAgePerShare.add(ageReward.mul(1e12).div(lpSupply));
        }
    ...
    // Update reward variables of the given pool to be up-to-da
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ageReward = multiplier.mul(agePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        age.mint(devaddr, ageReward.div(10));
        age.mint(address(syrup), ageReward);
        pool.accAgePerShare = pool.accAgePerShare.add(ageReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for AGE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {

        require (_pid != 0, "deposit AGE by staking");
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, "withdraw AGE by unstaking");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake AGE tokens to MasterChef
    ...
    // Withdraw AGE tokens from STAKING.
    ...
    function safeAgeTransfer(address _to, uint256 _amount) internal {
        syrup.safeAgeTransfer(_to, _amount);
    }
